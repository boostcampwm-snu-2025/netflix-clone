...
### 0. 4주차에 하지 못한 것들 ㅠㅜ
- [ ] !important 설계서 작성!
  - 1. 지금까지 진행한 내용에 대해서 어떻게 설계했는지 작성
  - 2. 이번 주 구현한/할 내용 설계서 작성
- [ ] 좋아요 버튼 구현
  - [ ]1. 버튼 위에 hover 시 반응하도록 JS 작성
  - [ ]2. hover 할 경우 hidden 속성을 없애면서 해당 위치에 넷플릭스처럼 뜨게 하기
  - [ ]3. 위치 잘 맞추기. 정확히 `slider-item`의 정중앙에 위치시켰는가?
- [ ] ES module 방식 이용해서 JS 파일들 구조 정리

### 2. 정적 & 동적 요소 수정사항
- [ ] ES module화 하기 위해 app.js 만들고 여기에 import 하는 방식으로 JS 파일 구현
- [ ] 검색창 구현
  - [ ]1. 검색 버튼을 누르면 거기에서 hidden된 검색창이 자연스럽게 나타나도록 하기
  - [ ]2. 검색어를 입력시 검색 결과가 하단에 노출되도록 설정 (가능하면)

### 3. 서버 환경 구성
- [ ] express 환경 적용
- [ ] JSON을 활용해 데이터의 고정된 값을 반환하도록 하기.
- [ ] 이거 솔직히 어떻게 하는지 모르겠으니 AI 이용 학습하기


### 4. 학습 내용 정리
- [ ] 구현한 내용들 설명할 수 있을 때까지 복습!!
- [ ] JS 구현하면서 AI로 짰던 부분들 다시 짜보기 or 더 나은 방식으로 코딩하기
- [ ] 이번 JS 구현에서 사용되었던 함수들 다시 복습
- [ ] JS 구현 과정에서 알지 못했던 개념들 (콜백 함수, fetch 등) 복습하기
- [ ] html, css에서 날 괴롭게 했던 `grid/flex`와 `position: absolute/relative` 정복하기
- [ ] 이번 과제에서 AI로 구현한 부분들 내용 정리 및 복습
  - [ ] 1. 슬라이더 부분 구현(무한 슬라이딩 등) 따로 해보기
  - [ ] 2. fetch 데이터로 받아오는 부분 내용 학습 필요
  - [ ] 3. 서버환경 구성 복습
