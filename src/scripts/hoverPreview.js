export function initHoverPreview() {
    ...
    container.addEventListener('mouseover', (e) => {
        ...
        clearHide();
        // if moving to a different item, reset timer
        if (showTimer) { clearTimeout(showTimer); showTimer = null; }
        const targetItem = item;
        showTimer = setTimeout(() => {
            // only show if still hovering the same item
    ...
    function applyRatingUI(rating) {
        // Main like button icon reflects current rating
    ...
    rateOptions.forEach((btn, idx) => {
        btn.addEventListener('click', (e) => {
            ...
            setRating(key, value);
            applyRatingUI(value);
            // close the menu shortly after selection
            setTimeout(() => {
                rateMenu.classList.remove('is-open');
                rateMenu.setAttribute('aria-hidden', 'true');
            }, 80);
